    ...
    public class IcecastSource : MonoBehaviour
    {
        ...
        public void Connect()
        {
            ...
            this.icecastWriter = new IcecastWriter()
            {
                ...
                Public = this._public,
                UserAgent = string.IsNullOrWhiteSpace(this.userAgent) ? "AudioStream " + About.versionNumber : this.userAgent
            };
            ...
            if (this.icecastWriter != null && this.icecastWriter.Connected)
            {
                switch (this.codec)
                {
                    ...
                    case IcecastSourceCodec.OGGOPUS:
                        ...
                        // AudioSource might have been already started before Connect
                        if (this.opusOggWriteStream != null)
                            this.opusOggWriteStream.WriteSamples(data, 0, dlength);

                        break;

                    case IcecastSourceCodec.PCM16:

                        AudioStreamSupport.UnityAudio.FloatArrayToPCM16ByteArray(data, (uint)dlength, ref this.bArr);
